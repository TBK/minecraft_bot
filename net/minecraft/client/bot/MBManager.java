...
public class MBManager {
	...
	public static MBManager searchAndReach(MinecraftBot mb, int[] items) {
		...
		manager.botList.add(manager.mb.new BotPath(manager.mb.new MBVec(0, -1,
				0), items, "MBManager Search and Reach Path", true));
		manager.macroId = 3;
		manager.step = 1;
	...
	public MANAGER_RETURN run() {
		 ...
		 */
		if (((MBPlayerControllerMP) (mb.mc.playerController)).flag)
			return MANAGER_RETURN.PLAYERCONTROLLER_ERROR;
	...
	private boolean runOptions() {
		if (mb.mc.thePlayer.getBrightness(1) < mb.TORCH_THRESHOLD
				&& mb.tools.countItems(mb.ITEMS_TORCH) > 0
				&& mb.new MBVec(0, -1, 0).getId() == 0) {
			botList.add(mb.new BotBuilder(mb.new MBVec(0, -1, 0),
					mb.ITEMS_TORCH));
			return true;
		}
		return false;
	...
	private void runSearchAndReach() {
		...
		if (step == 1 && botList.peek() != null
				&& botList.peek() instanceof BotPath) {
		...
		if (botList.size() == 0) {
			...
			if (step == 2 || GRAVEL > 0) {
				if (target.distanceTo() > MAX_MINING_DISTANCE) {
					...
					if (mb.scanner.scanVec(target.getVec(0, 1, 0), new int[] {
							12, 13 })) {
						botList.add(mb.new BotMiner(target.getVec(0, 1, 0)));
						GRAVEL = 15;
					} else if (!mb.scanner.scanVec(target.getVec(0, 1, 0),
							mb.BLOCKS_EMPTY_LAVA_WATER_FIRE))
						botList.add(mb.new BotMiner(target.getVec(0, 1, 0)));
					else {
			...
			if (step == 3) {
				if (mb.scanner.scanVec(target.getVec(0, -1, 0),
						mb.BLOCKS_EMPTY_LAVA_WATER_FIRE)) {
	...
	private void runChopStuff() {
		try {
			...
			if ((step == 4 || step == 5) && botList.size() == 0) {
				/*
				 * If one block over target is searched id, mine it (After
				 * coming back to original position)!
				 */
				if (Arrays.binarySearch(items, target.getId(0, 1, 0)) >= 0
						&& target.distanceTo(mb.new MBVec(true)) <= MAX_MINING_DISTANCE - 2) {
					target = target.getVec(0, 1, 0);
					step = 6;
				} else {
					botList.add(mb.new BotPath(mb.new MBVec(0, -1, 0), items,
							"MBManager Chopwood step 4", true));
					step = 1;
			...
			if (step == 3 && botList.size() == 0) {
				...
				if (target.getId(0, -1, 0) == 0 || target.getId(0, 1, 0) == 0) {
					botList.add(mb.new BotMove(target.getVec().aH(), true,
							"MBManager Chopwood step 3"));
					step = 4;
				} else
					step = 5;
			...
			if (step == 1 && botList.peek() instanceof BotPath
					&& ((BotPath) botList.peek()).path != null) {
				target = ((BotPath) botList.pop()).path.firstElement();
				botList.add(mb.new BotMove(target, false,
						"MBManager Chopwood step 1"));
				step = 2;
